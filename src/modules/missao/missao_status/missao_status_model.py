...
class MissaoStatusModel(Base):
    __tablename__ = "tb_missao_status"
    __table_args__ = {"schema": "sigeop"}
    
    cod_missao_status: Mapped[int] = mapped_column(Integer, primary_key=True)
    sig_missao_status: Mapped[str] = mapped_column(String(60), nullable=False, unique=True)
    dsc_missao_status: Mapped[str] = mapped_column(Text, nullable=False)
    ...
    missoes: Mapped[list["MissaoModel"]] = relationship(back_populates="status")
