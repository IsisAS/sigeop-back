...
class PlanoOperacaoModel(Base):
    __tablename__ = "tb_plano_operacao"
    __table_args__ = {"schema": "sigeop"}

    cod_plano: Mapped[int] = mapped_column(ForeignKey("sigeop.tb_plano.cod_plano"), primary_key=True)
    cod_operacao: Mapped[int] = mapped_column(ForeignKey("sigeop.tb_operacao.cod_operacao"), primary_key=True)
